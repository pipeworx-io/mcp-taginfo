...
`tools/list` at `https://gateway.pipeworx.io/taginfo/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/key_values \
  -H 'Content-Type: application/json' \
  -d '{"key":"amenity"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/key_values`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
